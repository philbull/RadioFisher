...
def flux_redshift(z):
    ...
    z = np.atleast_1d(z)
    nu = NU_LINE / (1. + z)
    if 4\pi \int_{z_min}^{z_max} dN/dz dzvnucrit[ID] is not None:
...
for sarea_in in sarea_vals:
    ...
    # Save n(z) and b(z) to file
    #fname = "nz_%s_800_1300_%d.dat" % (name[ID], sarea_in) # FIXME
    #np.savetxt( fname, np.column_stack((zc, zmin, zmax, nz, b)) )
    #print "Saved binned n(z) and b(z) in %s" % fname
    ...
    print "%10s %d %6.4f %6.4f %6.4f %6.4f %6.4f %3.3e" % \
    (name[ID], sarea_in, np.log10(p[0]), p[1], p[2], pb[0], pb[1], np.sum(nz * vol))
    
    # Plot n(z)
    frac = float(sarea_vals.index(sarea_in)) / float(len(sarea_vals))
    ax1.plot(zc, nz, marker='.', color=cm.rainbow(frac), label="%d" % sarea_in)
    
    # Comparison plot of dndz, bias, and fitting function
    #P.subplot(211)
    #ax1.plot(_z, dndz_expt(_z))
    #ax1.plot(_z, p[0] * _z**p[1] * np.exp(-p[2]*_z))

    #P.subplot(212)
    #ax2.plot(_z, bias_expt(_z))
    #ax2.plot(_z, pb[0] * np.exp(pb[1]*_z))

if DEBUG_PLOT:
    ax1.set_yscale('log')
    ax1.legend(loc='upper right', prop={'size':'x-small'}, ncol=3, frameon=False)
    ax1.plot(zc, 1e-4 / D(zc)**2., color='k', lw=2.)
    ax1.set_ylabel("n(z) [Mpc^-3]")
    ax1.set_xlabel("z")
    P.show()
