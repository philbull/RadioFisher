...
for k in range(len(names)):
    ...
    print "sigma(h) =", np.sqrt( Finv2[rf.indexes_for_sampled_fns(6, zc.size, zfns),rf.indexes_for_sampled_fns(6, zc.size, zfns)] )
    
    # Fix w0,wa,gamma
    F_eos3, lbls3 = rf.combined_fisher_matrix( F_eos_list, 
                                                    #exclude=[2,4,5,6,7,8,9,12], 
    ...
    w, h, ang, alpha = rf.ellipse_for_fisher_params(px3[0], py3[0], F_eos3, Finv=Finv3)
    ellipses = [matplotlib.patches.Ellipse(xy=(x, y), width=alpha[kk]*w, 
