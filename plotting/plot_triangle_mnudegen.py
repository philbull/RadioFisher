#!/usr/bin/python
"""
Make a triangle plot for massive neutrinos.
"""
import numpy as np
import pylab as P
...
names = ['cexptLmnu020', 'cexptLmnu010', 'cexptLmnu005']
fidmnu = [0.2, 0.1, 0.05]
labels = ['Facility (0.2)', 'Facility (0.1)', 'Facility (0.05)']
...
for k in _k:
    ...
    params = ['h', 'omegaDE', 'omegak', 'w0', 'wa', 'n_s', 'sigma8', 'Mnu'][::-1]
    label = ["$h$", "$\Omega_\mathrm{DE}$", "$\Omega_K$", "$w_0$", "$w_a$", "$n_s$", "$\sigma_8$", r"$M_\nu$"][::-1]
    
    fid = [ cosmo['h'], cosmo['omega_lambda_0'], 0., cosmo['w0'], cosmo['wa'], cosmo['ns'], cosmo['sigma_8'], fidmnu[k] ][::-1]
...
row_step = [0, 0, 0, 2, 2, 2, 2, 2]
col_step = [2, 2, 2, 0, 1, 1, 1, -1]
