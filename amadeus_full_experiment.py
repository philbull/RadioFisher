...
#INPUT AS:
#mpirun -n <NPROCESSORS> python full_experiment.py <EXPID> <SAREA> <BINTYPE> <BINWIDTH/NUMBER> <integration_time> <kmax> <N_kbins>
#where <EXPID> is the index of the experiment in the list expt_list below,
#<SAREA> has to be in degrees
#<BINTYPE> 0:dz const, 1:dr const, 2:dnu const
#<BINWIDTH/NUMBER> depends on the option chosen for bin type. for z and nu the bin width is given, BUT FOR R THE NUMBER OF BINS, NOT BIN WIDTH
#<integration_time> [hours ] ttot total integration time in [hours]
#<kmax> [Mpc^-1] kmax in the Fisher integrals
#<N_kbins> simply the number of kbins to be used.
#All of these parameters will enter the file name as follows:
#root = "output/" + survey_name + binning + binwidth + integration_time + nkbins
...
EXPT_LABEL = "_amadeus" #"_mg_Dz_kmg0.01"
...
expt_list = [
    ...
    ( 'ihirax', e.HIRAX),                    # 62
    #( 'ihirax', e.hirax),                   # 63
    #( 'icbasehirax',      e.cb_hirax),        # 64
    #( 'icbasehirax_2P',   e.cb_hirax_2P),     # 65
    #( 'icbasehirax_3P',   e.cb_hirax_3P),     # 66
    #( 'icbasehirax_10P',  e.cb_hirax_10P),    # 67
    #( 'icbase_Ndish190_hirax', e.cb_Ndish190_hirax),    #68
    #( 'ihexax', e.hexax),                   # 69
    #( 'SKA1MID1_plus_MEERKAT', e.SKA1MIDfull1_edited),  #70
    #( 'SKA1MID2_plus_MEERKAT', e.SKA1MIDfull2_edited),  #71
    #( 'MeerKATb1',             e.MeerKATb1_edited ),    #72 b1=UHF-band
    #( 'MeerKATb2',             e.MeerKATb2_edited ),    #73 b2=L-band
    #( 'icbasehirax_Ndish529',  e.cb_hirax_Ndish529),     #74
    #( 'icbasehirax_Ndish132',   e.cb_hirax_Ndish132),    #75
    #( 'icbasehirax_Ndish256',   e.cb_hirax_Ndish256),    #76
    #( 'MeerKATb1_noNoise',      e.MeerKATb1_noNoise),   #77
    #( 'MeerKATb2_noNoise',      e.MeerKATb2_noNoise),   #78
    #( 'iCVlimitedz0to3',        e.CVlimited_z0to3),   #79
    #( 'iCVlimitedz2to5',        e.CVlimited_z2to5), #80
    #( 'icbasehirax_kmax014',        e.cb_hirax_kmax014), #81
]
...
if len(sys.argv) > 1:
    ...
    try:
        ...
        pass
    try:
        bintype=int(sys.argv[3])
        bintype_width=float(sys.argv[4])
    except:
        bintype = None
        bintype_width = None
    ...
    try:
        ...
        kmax = 130. #default in fisher()

    try:
        kbin_number=int(sys.argv[7])
    except:
        kbin_number=None
else:
...
if bintype==0:
    zs, zc = rf.zbins_equal_spaced(expt_zbins, dz=bintype_width)
    root=root+'_zbin'+str(bintype_width)
elif bintype==1:
    bintype_width=int(bintype_width)
    zs, zc = rf.zbins_const_dr(expt_zbins, cosmo, bins=bintype_width)
    root=root+'_nrbin'+str(bintype_width)
elif bintype==2:
    zs, zc =  rf.zbins_const_dnu(expt_zbins, cosmo, dnu=bintype_width)
    root=root+'_nubin'+str(bintype_width)
else:
#    zs, zc = rf.zbins_equal_spaced(expt_zbins, dz=0.1)
    zs, zc =  rf.zbins_const_dnu(expt_zbins, cosmo, dnu=60.)

if integration_time is not None:
    expt['ttot']=integration_time * experiments.HRS_MHZ # conversion from survey time to total integration time
    root=root+'_ttot'+str(np.int(integration_time))+'hours'

if kmax != 130.:
    root = root+'_kmax'+str(kmax)+'invMpc'


# Define kbins (used for output)
if kbin_number is not None:
    kbins = np.logspace(np.log10(0.01), np.log10(1.0), kbin_number)
    root=root+'_nkbin'+str(kbin_number)
else:
    kbins = np.logspace(np.log10(0.01), np.log10(1.0), 61)
    # print "K from 0.01 to 0.15 \n"
    # root = root + "_k001to015"
    # kbins = np.logspace(np.log10(0.01), np.log10(0.15), 61)
    # kbins = np.logspace(np.log10(0.001), np.log10(50.), 91)##kbins = np.logspace(-4., 0., 81)
